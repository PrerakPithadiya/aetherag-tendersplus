...
### ROI Calculator Component (Interactive Teaser)
An interactive element allowing prospective users to estimate potential return on investment.
- **Layout**: 2-column card layout on desktop, single-column on mobile. Left side contains sliders and input controls, right side shows a large display metric with a circular or semi-circular efficiency gauge.
- **Controls**:
  - A farm size slider (range: 10 to 5,000 hectares, default: 250).
  - A crop type selection dropdown (options: Wheat, Corn, Soybeans, Cotton, default: Wheat).
  - A toggle for "Include Bid Automation".
- **Visuals**:
  - Card background: `surface-container-low` (`#f4f4f2`).
  - Text: Display metrics in `secondary` (`#3b6934`) with size `headline-md` or `display-lg-mobile`.
  - Slider track: `outline-variant` (`#c4c7c7`) height 2px, thumb: `secondary` (`#3b6934`) circular 16px.
  - Buttons: Call to action to unlock full platform features using `primary` (`#000000`) background and `on-primary` (`#ffffff`) text.
- **Interaction**: Sliders should update values in real-time, causing display numbers to count up or change with a smooth transition. Hovering over the card should lift it using the `.shadow-ambient-hover` animation.
